...
export default async function Page() {
  ...
  render(); // ← これを忘れると画面が変わらない
      ...
      <LessonSection id="react" {...at(REACT, "const [count, setCount]", "</div>")}>
        ...
        <h3>全部描き直したら、遅くならないのか</h3>

        <p>
          ここで当然の疑問が出ます。
          <strong>値が変わるたびに全部描き直していたら、遅いのでは。</strong>
        </p>

        <p>
          答えは、<strong>画面を全部作り直してはいない</strong>です。
          React がやっているのは 2 段階に分かれています。
        </p>

        <ol>
          <li>
            <strong>「こう表示してほしい」という設計図を作る</strong>。
            これは JavaScript のオブジェクトで、まだ画面ではありません
          </li>
          <li>
            <strong>前回の設計図と見比べて、違うところだけ</strong>{" "}
            実際の画面に反映する
          </li>
        </ol>
        ...
        <p>
          文字が 1 つ変わっただけなら、
          書き換わるのは<strong>その文字だけ</strong>です。
          設計図を作り直すのは軽い処理で、
          重いのは<strong>実際の画面をいじるほう</strong>。
          React は、その重いほうを最小限に抑えています。
        </p>

        <Callout variant="note" title="仮想 DOM という呼び方">
          <p>
            この「設計図」は<strong>仮想 DOM</strong> と呼ばれます。
            記事でよく見る言葉なので、名前だけ知っておいてください。
          </p>
          <p>
            仕組みの詳しい話は <strong>Part 7</strong> で扱います。
            ここでは<strong>「描き直す = 画面を全部作り直す、ではない」</strong>
            とだけ持っていってください。
          </p>
        </Callout>
