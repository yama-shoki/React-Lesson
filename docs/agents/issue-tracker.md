...
**残っているのは 1 つだけ: 全 57 章の光り方を実機で総ざらいすること。**

土台（`demo-card.tsx` / `render-box.tsx`）は react-dev と突き合わせて直し済み。
実機で確認したのは 4 章（usestate / server-components / debounce / lifting-state）で、
いずれも本文の主張どおりに動いていた。残り 53 章は未確認。

やり方は決めてある。ページ遷移すると window の状態が消えるので、
iframe に順次読み込ませて次を見る:

- 初期表示の計器がすべて `render 1` になっているか
- 読み込み後 2 秒たった時点で、静止しているのに光っているものがないか
- ❌/✅ の対で、片方だけが光る作りになっているか（要操作なので章ごとに個別）

`bun dev` を上げてから、ブラウザのコンソールで iframe を作って回すだけでよい。
57 章 × 約 3 秒で 3 分ほど。

過去に見つかった型の不具合は 2 つとも「実機で見ないと分からない」ものだった
（初回マウントを数えていた / RenderBox が親の巻き添えで光っていた）ので、
この総ざらいは省かないこと。
